...
def natural_sort_key(path):
    ...
    return [int(token) if token.isdigit() else token.lower() for token in re.split(r'(\d+)', basename)]

def annotation_to_seed(folder_path, output_path):
    # フォルダ内のアノテーション画像をまとめてseed画像にする
    file_paths = glob.glob(os.path.join(folder_path, '*_FramesForAnnotation.tif'))


    file_paths.sort(key=natural_sort_key)  # 数値を考慮した自然順ソート

    if not file_paths:
        raise FileNotFoundError(f"対象ファイルが見つかりません: {folder_path}")
    ...
    for file_path in file_paths:
        ...
        if merge_frames is None:
            merge_frames = labels_eroded
        else:
            merge_frames += labels_eroded
