...
from .Graph import create_graph, find_previous_red_edge, plot_graph
...
def main(label_number: int = 1,
    ...
    if save_middle_data:
        save_volume(os.path.join(output_dir, os.path.splitext(volume_file)[0] + "_overlay.tif"), extracted_volume)
        save_volume(os.path.join(output_dir, os.path.splitext(volume_file)[0] + "_overlay.npy"), extracted_volume)
    ...
    # Step4: 根管の二値化画像を編集し、 1 Seed 1 Label とする
    labeled_root_canal = labeled_for_root_canal(root_canal_binary, dilation_iterations=10)
    seed_root_canal = edit_root_canal(labeled_root_canal, volume)
    if debug:
        save_volume(os.path.join(output_dir, os.path.splitext(volume_file)[0] + "_seed_root_canal.tif"), seed_root_canal.astype(np.uint16))
        return
...
if __name__ == "__main__":
    ...
    for label_number in range(1, 4):
        ...
        input_dir  = fr"./study/Watershed/t-oe/20260716_NR_Label/Label_{label_number}"
        output_dir = None
